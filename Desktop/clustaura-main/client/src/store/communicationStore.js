...
const useCommunicationStore = create((set, get) => ({
    ...
    initSocket: () => {
        ...
        newSocket.on('incoming_call', (data) => {
            ...
            set({
                callState: {
                    inCall: false,          // ← Stay false so ringing UI shows (not call overlay)
                    callStatus: 'incoming', // ← GlobalCallManager checks this
                    roomId: data.roomId,
    ...
    startCallGlobal: (roomId, type, participants) => {
        const { socket } = get();
        if (!socket) {
            console.warn('[Call] Socket not ready — cannot start call.');
            return;
        }
        ...
        // participants is required for DM calls (array of target userIds)
        // For channel calls the server resolves participants via the channel's team
        console.log(`[Call] Starting ${type} call in room ${roomId}, participants:`, participants);
        socket.emit('start_call', { roomId, type, userId, participants: participants || [] });
