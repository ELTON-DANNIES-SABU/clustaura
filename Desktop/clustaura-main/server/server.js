    ...
    socket.on('start_call', async ({ roomId, type, participants, userId }) => {
        try {
            ...
            if (usersToNotify.length === 0) {
                console.warn(`[CALL] ⚠️  No users to notify for room ${roomId}. Check that 'participants' was passed from client for DM calls.`);
            }
            console.log(`[CALL] Starting call in room ${roomId}, notifying ${usersToNotify.length} users. Initiator: ${socket.user.firstName}`);
